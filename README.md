...
エディタのメニューから `GMornDebugMenu` を選ぶと、デバッガパネルのタブと同じ項目一覧・監視値を独立したウィンドウで開ける。パネルを毎回開き直さず、常に見える場所へ置いておきたいときに使う。

`Godot エディタ → プロジェクト → ツール → GMornDebugMenu`（`EditorPlugin.add_tool_menu_item()` で足している場所。バージョンによってメニュー名が変わることがある）。

- 実行中のプロセスがあれば、そのままデバッガパネルのタブと同じ内容が届く。押す・つまむ・選ぶ・入り切りの操作も同じように実行中のゲームへ届く
- 実行していない、またはまだ繋がっていないときは「実行中プロセスなし」と表示され、エラーは出さない
- ウィンドウは閉じても作り直さない。次に開いたときはそのときの接続状況へ合わせ直す

中身は `gmorn_debug_menu_debugger_tab.gd` をそのまま使う。繋ぐ先のセッションは `gmorn_debug_menu_debugger_plugin.gd` の `bind_window()` が選ぶ。
